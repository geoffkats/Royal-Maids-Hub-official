        ...
        <div class="bg-gradient-to-r from-[#3B0A45] to-[#512B58] border border-[#F5B301]/30 rounded-xl p-6 shadow-lg">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="xl" class="text-white">{{ __('Edit Evaluation') }}</flux:heading>
                        <flux:subheading class="mt-1 text-[#D1C4E9]">{{ __('Evaluation for') }} {{ $evaluation->maid?->first_name }} {{ $evaluation->maid?->last_name }}</flux:subheading>
                    </div>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <div>
                    ...
                    <div class="px-3 py-2 bg-[#3B0A45] border border-[#F5B301]/30 rounded-lg text-[#D1C4E9]">
                        <flux:badge color="{{ $status === 'approved' ? 'green' : ($status === 'rejected' ? 'red' : 'yellow') }}" size="sm">
                            {{ ucfirst($status) }}
                        </flux:badge>
                        @if($status === 'pending')
