<div class="min-h-screen bg-[#3B0A45] dark:bg-[#3B0A45] p-6">
        ...
        <div class="bg-gradient-to-r from-[#3B0A45] to-[#512B58] border border-[#F5B301]/30 rounded-xl p-6 shadow-lg mb-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <flux:heading size="xl" class="text-white">{{ __('Edit Training Program') }}</flux:heading>
                        <flux:subheading class="mt-1 text-[#D1C4E9]">{{ $program->program_type }}</flux:subheading>
                    </div>
                </div>
        ...
        <form wire:submit.prevent="update" class="space-y-6">
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-[#F5B301]/20">
                ...
                <flux:button type="submit" class="bg-[#F5B301] hover:bg-[#F5B301]/80 text-[#3B0A45] font-medium px-6 py-2 rounded-lg transition-colors shadow-lg hover:shadow-xl">
                    {{ __('Update Program') }}
                </flux:button>
            </div>
        </form>
